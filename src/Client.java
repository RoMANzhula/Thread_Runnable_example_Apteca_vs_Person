public class Client implements Runnable, RandomCount, RandomDrug, Waiter { //создаем класс Клиент, который реализует ряд интерфейсов
    ...
    public void run() { //обязательный(т.к. мы реализуем интерфейс Runnable) к переопределению метод с логикой для выполнения
        ...
        while (!Main.isStopped) { //создаем цикл с условием (пока isStopped = false)
            Main.drugsController.buy(getRandomDrug(), getRandomCount()); //для статического(одно на всех) обьекта, который
            //находится в классе Main, вызываем метод купить(аргументами конструктора выступают дефолтные(реализованные) методы
